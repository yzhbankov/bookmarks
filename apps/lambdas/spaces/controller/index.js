import { ReadSpace, CreateSpace, DeleteSpace } from '../usecases/index.js';
...
export default async function controller(method, event) {
    ...
    switch (method) {
        ...
        case 'POST': {
            return Controller.post({ ...body, owner });
        }
        case 'DELETE': {
...
class Controller {
    static async get(params) {
        return makeRequestHandler(ReadSpace, params)
    }
    static async post(params) {
        return makeRequestHandler(CreateSpace, params)
    }
    static async del(params) {
        return makeRequestHandler(DeleteSpace, params)
    }
}
